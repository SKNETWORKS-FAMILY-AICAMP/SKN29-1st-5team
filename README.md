...
- **윤승혁** : <br>
파이썬 생기초부터 시작하는 수업을 12일이라는 짧은 기간동안 수강하고 진행한 프로젝트였는데, 생각보다 퀄리티가 높은 작품이 나와서 뿌듯한 경험이었습니다. Streamlit은 강의를 통해 처음 알게 된 프레임워크였는데 어렵지 않은 코드로 깔끔한 화면을 구성할 수 있어서 매우 좋은 프레임워크라는 생각이 들었습니다. 그리고 OpenAPI를 통해 데이터베이스에 실시간 데이터를 저장할 수 있다는 점이 인상깊었고, 4명의 팀원이 동일한 데이터베이스에서 공동으로 작업함으로써 협업 경험을 쌓은 것이 유용한 자산이 된 것 같습니다. 그리고 지식이 완벽하지 않더라도 일단 시작해보는 용기가 중요하다는 것을 깨달았습니다.
아쉬웠던 점은 프로젝트 진행 기간이 짧고 주제 선정이 오래 걸려서 기획 단계가 엉성한 느낌이 있었던 것 같습니다. 프로젝트를 완성하고 ERD를 확인해보니 휴게소 쪽 테이블들은 관계가 잘 맺어진 편이었는데 그 외 테이블들은 서로 연관없이 따로 노는 느낌이 들어서, 처음부터 ERD 설계를 제대로 해놓고 프로젝트를 진행했으면 좋았겠다는 생각이 들었습니다. 물론 첫 프로젝트다보니 처음부터 기획을 완벽하게 하기보다는 일단 이것저것 시도해보는게 더 좋은 방향이었을수도 있는데, 이후의 프로젝트에서는 기획에도 더 신경을 쓰는 것이 좋을 것 같습니다.
- **최지용** : <br>
처음 시작하는 비전공자 입장에서 최대한 팀원들에게 민폐끼치지 않도록 사전 학습을 하고 진행을 하게되어 저의 역할임무를 할 때 큰 무리없이 진행할 수 있었습니다.더불어 기획 과정에서 팀원 모두 창의적인 아이디어를 잘 끌어내어서 주제에 적합한 작업물에 접근하기 용이했습니다. 프로벡트 막바지에는 작업이 진행 될 때 각자 작업한 데이터를 같은 네트워크망에 접속하여 실시간으로 자료를 주고받은 덕에 원활하게 작업을 마칠 수 있었다고 생각합니다.<br>
아쉬웠던 점은 아직 스킬이 많이 부족한 탓에,  에러가 발생했을 때 해결하는 시간이 남들보다 길었습니다, 2차 프로젝트 전 까지 예복습을 꾸준히 이어나가면서 문제없이 다음 프로젝트를 진행할 수 있도록 해야겠습니다.<br>짧은 프로젝트 기간동안 한정된 주제로 진행을 했다보니, 팀원들과 소통 및 준비시간에 시간을 많이 할애하지 못하여 기획단계에서 버벅이면서 진행이 원활하게 되지 못한 느낌이 들었습니다.<br>다음 프로젝트에서는  이런 문제가 생기지 않도록 과정의 순서를 상기하며 잘 진행해야겠다고 느꼈습니다. 
- **한예나** : <br>
약 2주 동안 배웠던 것들을 활용해 직접 구현해보면서 저절로 복습까지 되는 뜻 깊은 시간이었습니다. 실무에서 제일 중요한 협업 능력을 기를 수 있어 좋았고 팀원들과 갈등 없이 원활하게 소통한 덕분에 주어진 기간에 비해 퀄리티 높은 결과물이 나왔습니다.
<br> 아쉬운 점은 전부 처음 접하는 도구들이어서 프로젝트 진행 시 팀원들의 도움을 정말 많이 받았습니다. 추후 프로젝트 진행 시에는 충분한 예복습을 통해 제가 맡게 되는 역할이 더 많았으면 합니다.
